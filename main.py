...
def get_with_retry(url, config, driver, retries=3, delay=1):
    # Get the URL with retries and delay using Selenium
...
def transform(page_source):
    # Parsing the job card info (title, company, location, date, job_url) from the Selenium page source
    joblist = []
    try:
        soup = BeautifulSoup(page_source, 'html.parser')
        divs = soup.find_all('div', class_='base-search-card__info')
    except Exception as e:
        print(f"Empty page, no jobs found, error: {e}")
        return joblist
    for item in divs:
...
def get_jobcards(config, driver):
    # Function to get the job cards from the search results page
    all_jobs = []
    for k in range(0, config['rounds']):
        for query in config['search_queries']:
            keywords =  urllib.parse.quote_plus(query['keywords'])  # URL encode the keywords
            location =  urllib.parse.quote_plus(query['location'])  # URL encode the location
            for i in range(0, config['pages_to_scrape']):
                ...
                page_source = get_with_retry(url, config, driver)
                if page_source:
                    jobs = transform(page_source)
...
def main(config_file):
    ...
    try:
        ...
        all_jobs = get_jobcards(config, driver)
        ...
        if len(all_jobs) > 0:
            for job in all_jobs:
                ...
                desc_page_source = get_with_retry(job['job_url'], config, driver)
                if desc_page_source:
                    job['job_description'] = transform_job(desc_page_source)
                    job['job_html'] = desc_page_source
